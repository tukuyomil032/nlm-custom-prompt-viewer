...
function printListFailures(language: LanguageCode, failures: PromptListFailure[]): void {
  ...
  for (const failure of failures.slice(0, 5)) {
    console.warn(
      chalk.yellow(
        `- ${failure.artifactId} [${failure.artifactType}] ${failure.artifactTitle} (${failureReasonLabel(language, failure.reason)})`,
      ),
    );
  }
...
async function runPromptList(language: LanguageCode, deps: RuntimeDeps, input: PromptListInput) {
  ...
  if (input.options.json) {
    const results = await deps.service.listPrompts(notebookId, { type, limit });
    console.log(JSON.stringify(results, null, 2));
    return;
  }

  const detailed = await deps.service.listPromptsDetailed(notebookId, { type, limit });
